...
namespace SPEENTITY.C_34_Mgt
{
    public class SalPurAccComboManager
    {
        ...
        public List<SalPurAccCombo> GetSalPurAccByMon(string date) 
        {
        ...
        public List<SPEENTITY.C_34_Mgt.EClassSalPurAcc> GetSalPurAccByMonLP(string date)
        {
            List<SPEENTITY.C_34_Mgt.EClassSalPurAcc> lst = new List<SPEENTITY.C_34_Mgt.EClassSalPurAcc>();
            string comcod = ObjCommon.GetCompCode();
            SqlDataReader dr = _ProAccess.GetSqlReader(comcod, "SP_REPORT_DASH_BOARD_INFO_LP", "MONTHWISEDASHBOARD", date, "", "", "", "", "", "", "", "");

            while (dr.Read())
            {
                SPEENTITY.C_34_Mgt.EClassSalPurAcc details = new SPEENTITY.C_34_Mgt.EClassSalPurAcc(dr["yearmon"].ToString(), dr["yearmon1"].ToString(), Convert.ToDouble(dr["ttlsalamt"].ToString()), Convert.ToDouble(dr["collamt"].ToString()), Convert.ToDouble(dr["ttlpuramt"].ToString()), Convert.ToDouble(dr["tpayamt"].ToString()), Convert.ToDouble(dr["dram"].ToString()), Convert.ToDouble(dr["cram"].ToString()));
                lst.Add(details);
