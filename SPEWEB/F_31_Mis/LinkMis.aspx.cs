...
{
    public partial class LinkMis : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                this.lblHeadtitle.Text = (this.Request.QueryString["Type"].ToString() == "BillConfirmation") ? "Bill Confirmation Information"
                    : (this.Request.QueryString["Type"].ToString() == "ResCostDet") ? "Cost Details (Resource)" : "";
                this.SelectView();
            }
        }
        ...
        private void ShowBillTracking()
        {
            ...
            ViewState.Remove("tblmis");
            string comcod = this.GetCompCode();
            string billno = this.Request.QueryString["billno"];

            DataSet ds1 = MisData.GetTransInfo(comcod, "SP_REPORT_REQ_STATUS", "RPTPURBILLTRACK", billno, "", "", "", "", "", "", "", "");
            if (ds1 == null)
            {
            ...
            ViewState["tblmis"] = ds1.Tables[0];
            this.Data_Bind();

        }

        private void ShowResCostDetails()
        {
            ...
            string ResCode = this.Request.QueryString["rescode"].ToString().Substring(0, 2);
            string txtdatefrm = this.Request.QueryString["frmdate"].ToString();
            string txtdateto = this.Request.QueryString["todate"].ToString();
            string level = "12";
            DataSet ds1 = MisData.GetTransInfo(comcod, "SP_REPORT_MIS", "RPT4COMPROJECTCOST", "", txtdatefrm, txtdateto, ResCode, level, "", "", "", "");
            if (ds1 == null)
            {
            ...
            this.lblResName.Text = this.Request.QueryString["resdesc"].ToString();
            this.lblfrmdate.Text = txtdatefrm;
            this.lbltodate.Text = txtdateto;
            ViewState["tblmis"] = ds1.Tables[0];
        ...
        {
            string comcod = this.GetCompCode();
            string billno = this.Request.QueryString["billno"].ToString();
            // ViewState.Remove("tblbillinfo");
            DataSet ds1 = MisData.GetTransInfo(comcod, "SP_REPORT_REQ_STATUS", "GETBILLNO", billno, "", "", "", "", "", "", "", "");
            if (ds1 == null)
            {
                this.lblbillno.Text = "";

            }
            this.lblbillno.Text = ds1.Tables[0].Rows[0]["billno1"].ToString().Substring(0, 11);
            this.lblProjectName.Text = ds1.Tables[0].Rows[0]["actdesc"].ToString();
            //  ViewState["tblbillinfo"] = ds1.Tables[0];
        }
        ...
        private void Data_Bind()
        {
            ...
            DataTable dt = (DataTable)ViewState["tblmis"];
        ...
        protected void PrintBillTracking()
        {
            //DataTable dt = (DataTable)ViewState["tblmis"];
            //Hashtable hst = (Hashtable)Session["tblLogin"];
            ...
            //ReportDocument rptpur = new RMGiRPT.R_14_Pro.rptPurchaseBillTk();
            ...
            //txtProjectName.Text = ((DataTable)Session["tblbillinfo"]).Rows[0]["actdesc"].ToString();

            //TextObject txtSupplierName = rptpur.ReportDefinition.ReportObjects["txtSupplierName"] as TextObject;
            //txtSupplierName.Text =((DataTable)Session["tblbillinfo"]).Rows[0]["ssirdesc"].ToString();


            //TextObject txtreqno = rptpur.ReportDefinition.ReportObjects["txtbillno"] as TextObject;
            //txtreqno.Text = "Bill  No: " +this.lblbillno.Text.Trim();

            //TextObject rpttxtMRFno = rptpur.ReportDefinition.ReportObjects["txtRefNo"] as TextObject;
            //rpttxtMRFno.Text = "Ref No: " + ((DataTable)Session["tblbillinfo"]).Rows[0]["billref"].ToString();

            //TextObject txtFDate1 = rptpur.ReportDefinition.ReportObjects["txtreqdate"] as TextObject;
            //txtFDate1.Text = "Bill Date: " + Convert.ToDateTime(((DataTable)Session["tblbillinfo"]).Rows[0]["billdat"].ToString()).ToString("dd-MMM-yyyy");
