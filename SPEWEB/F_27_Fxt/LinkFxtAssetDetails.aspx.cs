...
namespace SPEWEB.F_27_Fxt
{
    public partial class LinkFxtAssetDetails : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            ...
            string spcfcod = this.Request.QueryString["spcfcod"].ToString();
            ...
            DataSet ds1 = PurData.GetTransInfo(comcod, "SP_ENTRY_FIXEDASSET_INFO", "FXTASSETDETAILS", pactcod, code, spcfcod, "", "", "", "", "", "");
            if (ds1 == null)
                return;
        ...
        private void SaveValue()
        {
            ...
            for (int i = 0; i < dt2.Rows.Count; i++)
            {
                ...
                ((Label)this.gvProSlInfo.FooterRow.FindControl("lblFqty")).Text = totalqty.ToString();
        ...
        protected void lUpdate_Click1(object sender, EventArgs e)
        {
            ...
            string spcfcod = this.Request.QueryString["spcfcod"].ToString();
            int stockqty = Convert.ToInt32(this.Request.QueryString["Qty"].ToString());
            //string pactcode = this.ddlProjectName.SelectedValue.ToString();  // this.Request.QueryString["pactcode"].ToString();
            int givenqty = Convert.ToInt32(((Label)this.gvProSlInfo.FooterRow.FindControl("lblFqty")).Text);
            if (givenqty- stockqty > stockqty) // check last stock and new entry then check final stock by Safiul Alam, Sr Software Eng
            {
                this.lmsg.Text = "Updated Fail Given Qty Is Wrong";
            }
            else
            {

                bool result = PurData.UpdateXmlTransInfo(comcod, "SP_ENTRY_FIXEDASSET_INFO", "INSERTUPDATEXTASSET", ds1,
                    null, null, "", rsircode, spcfcod, "", "",
                     "", "", "", "", "", "", "", "", "", "", "", "", "", "", "");
                string a= ds1.GetXml();

                if (!result)
                {
                    this.lmsg.Text = "Updated Fail";
                }
                else
                {
        ...
        protected void LinkButton1_Click(object sender, EventArgs e)
        {
            ...
            {
                int maxLavel = Convert.ToInt32(dt1.Compute("max([slno])", string.Empty));
                int slno = maxLavel + 1;//
        ...
        protected void gvProSlInfo_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (assetid.Length > 0)
                {
                    hlink1.NavigateUrl = "../F_27_Fxt/FxtAstGinf.aspx?Type=Fselection" + "&sircode=" + code + "&spcfcod=" + spcfcod + "&genno=" + assetid + "&slno=" + slno;
        ...
        protected void btnde_click(object sender, EventArgs e)
        {
            ...
            bool result = PurData.UpdateXmlTransInfo(comcod, "SP_ENTRY_FIXEDASSET_INFO", "DELETEALLOCATEDASST", null, null, null, Pactcod, rsircode, slno, spcfcod, "",
                 "", "", "", "", "", "", "", "", "", "", "", "", "", "", "");
