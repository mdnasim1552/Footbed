...
namespace SPEWEB.F_27_Fxt
{
    public partial class FxtAstGinf : System.Web.UI.Page
    {
        ...
        private void GetFixedAssetName()
        {
            ...
            var spcfcod = this.Request.QueryString["spcfcod"].ToString().Trim();

            Hashtable hst = (Hashtable)Session["tblLogin"];
            string comcod = hst["comcod"].ToString();
            string txtSProject = type == "Fselection" ? rcode + spcfcod : "%" + this.txtSrcFAst.Text + "%";
        ...
        protected void lUpdatPerInfo_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < this.gvFAstInf.Rows.Count; i++)
            {
                ...
                FAstData.UpdateTransInfo(comcod, "SP_ENTRY_FIXEDASSET_INFO", "INSERTORUPDATEFIXEDASSETINF", ASTUtility.Left(ssirCode, 12), Gcode, gtype, Gvalue, assetcod, (ASTUtility.Right(ssirCode, 12)), "", "", "", "", "", "", "", "");
            }
        ...
        protected void FileUploadComplete(object sender, AsyncFileUploadEventArgs e)
        {
            ...
            {
                ssircode = ASTUtility.Left(this.ddlFAstName.SelectedValue.ToString(), 12);
                spcfcode = ASTUtility.Right(this.ddlFAstName.SelectedValue.ToString(), 12);
                string random = ASTUtility.RandNumber(1, 999999).ToString();
                string extension = Path.GetExtension(AsyncFileUpload1.PostedFile.FileName);
                AsyncFileUpload1.SaveAs(Server.MapPath("~/Uploads/FIXEDASSET/") + random + extension);

                Url = random + extension;
            }
            //Url = Url.Substring(0,(Url.Length-1));

            bool result = FAstData.UpdateTransInfo(comcod, "SP_ENTRY_FIXEDASSET_INFO", "FIXAIMAGEUPLOAD", ssircode, Url, assetcod, spcfcode, "", "", "", "", "", "", "", "", "", "");
