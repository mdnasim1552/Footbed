...
namespace SPEWEB.F_33_Doc
{
    public partial class GroupChat : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ((Label)this.Master.FindControl("lblTitle")).Text ="Group Task Center";
        ...
        {
            string comcod = this.GetComeCode();
            DataSet ds1 = User.GetTransInfo(comcod, "SP_ENTRY_PURCHASE_01", "PRJCODELIST", "%", "", "", "", "", "", "", "", "");
        ...
        public static string SaveGroupChat(string userid, string chatname, string actcode, string message, string probdate, string taskcod, string asinuser)
        {
            ...
            bool result = _DataEntry.UpdateTransInfo2(comcod, "SP_CHAT_MGT", "SAVE_AND_GET_CHAT", userid1, chatname, posteduser, posteddat, terminal, session, actcode, message, probdate, taskcod, asinuser);
...

        {
            ...
            List<SPEENTITY.C_34_Mgt.EclassGroupChat> list2 = result.Tables[0].DataTableToList<SPEENTITY.C_34_Mgt.EclassGroupChat>();
...

        {
            ...
            DataSet result = _DataEntry.GetTransInfo(comcod, "SP_CHAT_MGT", "GET_CHAT_MSG", chatno.Trim(), userid, "");
            List<SPEENTITY.C_34_Mgt.EclassChatMSG> list2 = result.Tables[0].DataTableToList<SPEENTITY.C_34_Mgt.EclassChatMSG>();
            var jsonSerialiser = new JavaScriptSerializer();
            var json = jsonSerialiser.Serialize(list2);
            return json;
        }
