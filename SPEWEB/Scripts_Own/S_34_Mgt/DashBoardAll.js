...
function GetMonthlyInfo() {
    ...
    $.ajax({
        type: "POST",
        async: true,
        url: '../Service/ASMX_34_Mgt/DashBoardService.asmx/GetMonthlySalCal',
        //url: "DashBoardAll.aspx/GetMonthlySalCal",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        data: Sys.Serialization.JavaScriptSerializer.serialize({ 'selectedDate': fromDate }),
...
function Populate(data) {
    ...
    $(data.d).each(function (index, value) {
        ...
        value.ttlsalamt == 0 ? Records[index][3] = '' : Records[index][1] = Math.ceil(value.ttlsalamt).toLocaleString('en-US', { minimumFractionDigits: 0 });
        ...
        //purchase
        value.tllpuramt == 0 ? Records[index][3] = '' : Records[index][3] = Math.ceil(value.tllpuramt).toLocaleString('en-US', { minimumFractionDigits: 0 }); 
        value.tpayamt == 0 ? Records[index][4] = '' : Records[index][4] = Math.ceil(value.tpayamt).toLocaleString('en-US', { minimumFractionDigits: 0 });
        grphRecords[index][3] = value.tllpuramt;
        grphRecords[index][4] = value.tpayamt;

        tpar += value.tllpuramt;
        tpay += value.tpayamt;
